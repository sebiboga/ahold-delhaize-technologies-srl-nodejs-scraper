...
## Project
AD/01 scraper for peviitor.ro (Node.js, ESM, Jest)
...
- **Only the API parsing logic in `index.js`** (`fetchJobsPage`, `parseApiJobs`) is AD/01-specific. The output shape (`mapToJobModel`, `transformJobsForSOLR`) must stay uniform across derived scrapers.
...
# E2E tests (real AD/01 API, SOLR conditional)
npm run test:e2e
...
- `tests/validate-ad01-jobs.js` — CI fast validator (HEAD only); thin wrapper over src/job-validator.js + solr.js
