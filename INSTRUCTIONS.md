...
This scraper extracts job listings from AD/01 careers page (Romania only) and imports them to peviitor.ro.

Target: https://www.ad01.com/en/jobs/romania
...
1. **Start with brand** - We know the brand (e.g., "AD/01")
...
8. **Scrape new jobs** - Extract jobs from AD/01 careers page (Romania)
...
> **Important**: Scraper does NOT delete jobs from other sources (ANOFM, etc). It only upserts AD/01 Careers jobs. Existing jobs are preserved.
...
3. **Scrape jobs** - Extract jobs from AD/01 careers API (Romania only)
...
company.js (validate company)
    ...
    │
    ▼ (if active)
scrape AD/01 API (jobs for Romania)
    │
    ▼
...
| `tests/validate-ad01-jobs.js` | CI fast validator (HEAD only); thin CLI over `src/job-validator.js` + `solr.js` |
...
| `tests/e2e/scraper.test.js` | End-to-end tests with real AD/01 API |
