...
| `src/job-validator.js` | Shared validation primitives - exports validateByHead(url), validateByContent(url, opts), DEFAULT_EXPIRED_KEYWORDS. Used by both `validate-jobs.js` and `tests/validate-ad01-jobs.js`. |
...
| `tests/company.json` | Mock ANAF company data for AD/01 used in unit tests |
| `tests/validate-ad01-jobs.js` | **AD/01-specific fast validator (used by CI).** HEAD requests only, hardcoded AD/01 CIF. Called nightly by `automation-testing.yml`. Supports `--dry-run` and `--delete`. |
...
| `tests/e2e/scraper.test.js` | E2E tests - full pipeline with real AD/01 API, ANAF, and SOLR |
...
| `ROBOTS.md` | robots.txt analysis and scraping policy for AD/01 Careers |
...
| `delete_request.json` | **Manual maintenance tool** — SOLR payload to delete ALL jobs for CIF 49544242. Use only when you need to wipe AD/01 jobs from SOLR entirely. Run with: `curl --user "${SOLR_AUTH}" "https://solr.peviitor.ro/solr/job/update?commit=true" -H "Content-Type: application/json" -d @delete_request.json` |
...
- Full workflow: validate company (ANAF+Peviitor) → scrape AD/01 → transform → upsert SOLR → generate docs/jobs.md