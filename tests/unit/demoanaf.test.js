...
describe('src/anaf.js', () => {
  ...
  describe('searchCompany', () => {
    it('should return array of companies for valid brand', async () => {
      mockFetch.mockResolvedValue(anafSearchResponse([
        { cui: 49544242, name: 'AHOLD DELHAIZE TECHNOLOGIES SRL', statusLabel: 'Funcțiune' }
      ]));
    ...
    it('should throw on HTTP error', async () => {
      ...
      await expect(anaf.searchCompany('AD/01')).rejects.toThrow('ANAF search error: 500');
    });
    ...
    it('should encode brand name in URL', async () => {
      ...
      await anaf.searchCompany('AD/01 SRL');
      expect(capturedUrl).toContain(encodeURIComponent('AD/01 SRL'));
    });
  });
  ...
  describe('getCompanyFromANAF', () => {
    it('should return company data for valid CIF', async () => {
      ...
      const data = await anaf.getCompanyFromANAF('49544242');
    ...
    it('should throw after exhausting retries', async () => {
      ...
      await expect(anaf.getCompanyFromANAF('49544242')).rejects.toThrow();
  ...
  describe('getCompanyFromANAFWithFallback', () => {
    it('should return fresh data when API works', async () => {
      ...
      const data = await anaf.getCompanyFromANAFWithFallback('49544242');
    ...
    it('should use cached data when API fails', async () => {
      ...
      const data = await anaf.getCompanyFromANAFWithFallback('49544242', CACHED_DATA);
    ...
    it('should throw when API fails and no cache available', async () => {
      ...
      await expect(anaf.getCompanyFromANAFWithFallback('49544242')).rejects.toThrow();
    });
  });
});
