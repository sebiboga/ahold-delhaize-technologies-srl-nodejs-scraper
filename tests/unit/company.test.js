...
describe('company.js', () => {
  ...
  describe('getCompanyData (no cache)', () => {
    it('should fetch AD/01 via direct CIF lookup and return company data', async () => {
      mockFetch.mockResolvedValueOnce(anafCompanyResponse(AD01_ANAF_RECORD));

      const result = await company.getCompanyData();

      expect(result).toHaveProperty('company', 'AHOLD DELHAIZE TECHNOLOGIES SRL');
      expect(result).toHaveProperty('cif', '49544242');
      expect(result).toHaveProperty('active', true);
      expect(result).toHaveProperty('anafData');
      expect(result.anafData.name).toBe('AHOLD DELHAIZE TECHNOLOGIES SRL');
    });
    ...
    it('should throw when ANAF returns no company name', async () => {
      mockFetch.mockResolvedValueOnce(anafCompanyResponse({ cui: 49544242, name: null }));
  ...
  describe('validateAndGetCompany', () => {
    ...
    it('should return company data with status active', async () => {
      mockFetch
        .mockResolvedValueOnce(anafCompanyResponse(AD01_ANAF_RECORD))
        .mockResolvedValueOnce(solrResponse(5, [
          { url: 'https://test.com/1', title: 'Job 1' },
          { url: 'https://test.com/2', title: 'Job 2' }
        ]))
        .mockResolvedValueOnce(peviitorResponse([{ company: 'AHOLD DELHAIZE TECHNOLOGIES SRL' }]));
    ...
    if (AD01_ANAF_RECORD.inactive) {
      it('should return inactive status when company is inactive', async () => {
        const inactiveRecord = { ...AD01_ANAF_RECORD, inactive: true };
